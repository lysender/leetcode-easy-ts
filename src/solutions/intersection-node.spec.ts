...
import {
  getIntersectionNode,
  getIntersectionNodeDiffNodeCounts,
...
describe('intersection-node', () => {
  ...
  describe('using length difference count', () => {
    describe('test1 - with intersect', () => {
      it('should pass', async () => {
        const list1 = createList([4, 1, 8, 4, 5]);
        const list2 = createList([5, 6, 1]);
        ...
        expect(getIntersectionNodeDiffNodeCounts(list1[0], list2[0])).toEqual(
          list1[2],
        );
      });
    });
